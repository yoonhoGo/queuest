...
새 화면과 기능을 만들 때 사용하는 디자인 기준이다. 2026-09-11 사용자가 승인한 **레트로 웹 미리보기**의 시각 언어와, 첨부 레퍼런스에서 파생한 **기본 테마**의 종이 보드 문법을 함께 관리한다. 기존 화면을 단순히 비슷한 색으로 칠하는 것이 아니라 아이콘, 카드 구조, 간격, 글자 위계와 상태 표현까지 이어간다.
...
- macOS 앱 아이콘은 제공된 금색·검정 로고를 사용하고, 메뉴 막대 아이콘은 하나의 넓은 검신 위에 중앙 점이 있는 채운 Queue 칸 세 개를 올린 단색 템플릿이다. 1× 크기에서 칸 사이 간격은 약 1px이며, 그 틈과 양끝에 중앙 홈이 있는 검신이 보여야 한다. 원본과 생성 스크립트는 `apps/desktop/src-tauri/icons/`에 둔다.
...
| 기본 테마 전용 오버라이드 | [default-theme.css](../apps/desktop/src/styles/default-theme.css) |
...
![별빛 모험 테마 420×640 브라우저 렌더](images/fantasy-theme-preview.png)

- 새 설치의 기본값은 첨부 레퍼런스를 옮긴 밝은 `기본` 테마다. 타이틀바의 설정 버튼에서 테마를 선택하며, 선택값은 현재 기기의 앱 저장소에 유지한다.
- `기본` 테마는 파란 모눈 배경, 밝은 종이 표면, 민트 타이틀바, 연결된 네 개의 탭, 금색 선택·진행 상태, 짙은 청록색 주요 동작을 사용한다. 실제 앱의 인박스·원정·캐릭터·설정 화면에 같은 카드 문법을 적용하며, 데이터와 동작은 다른 테마와 공유한다.
- `레트로`는 기존 테마다. 새 테마의 추가로 레트로의 토큰·각진 셸·화면 동작을 변경하지 않는다. 기존 저장값이 `retro`이면 계속 레트로로 시작한다.
- `별빛 모험`은 첨부된 판타지 퀘스트 레퍼런스의 구조를 분석해 옮긴 선택형 어두운 테마다. 단순 팔레트 교체가 아니라 말풍선형 셸, 독립된 네 개의 탭, 일러스트 진행 배너, 빠른 입력 행, 조밀한 퀘스트 행, 금빛 CTA와 인용 푸터를 한 세트로 적용한다. 데이터 구조와 실제 동작은 레트로 테마와 공유한다.
- 별빛 모험의 앱 셸은 평범한 둥근 창이 아니라, 상단 중앙의 작은 돌출부를 가진 **대화창형 퀘스트 패널**이다. 바깥은 한 겹의 푸른 윤곽, 안쪽은 얇은 하이라이트, 표면은 어두운 남색의 세 단계 깊이로 만든다. 모든 카드를 같은 둥근 표면으로 만들지 말고, 배너·목록·상태 보드의 깊이를 분명히 구분한다.
- 인박스 배너는 `src/assets/fantasy-quest-banner.png`의 앱 전용 원본 삽화를 사용한다. 레퍼런스의 인물·성·삽화를 복제하지 않으며, 삽화 안에는 텍스트나 UI를 넣지 않는다. 왼쪽 절반은 실제 제목을 올릴 수 있는 어두운 여백, 오른쪽은 숲의 모험가와 랜턴, 하단은 진행 막대를 읽을 수 있는 비네트 구조를 유지한다.
- 별빛 모험에서만 창·탭·카드·버튼의 모서리를 부드럽게 하고 그림자를 확산형 빛으로 바꾸는 것은 의도된 차이다. 포커스, 텍스트 대비, 상태를 말하는 텍스트와 아이콘은 두 테마 모두 유지한다.
- 판타지 퀘스트 행에 표시하는 배지는 실제 인박스 상태인 `대기` 또는 `완료`다. 레퍼런스의 임의 카테고리와 XP 숫자를 데이터가 없는 인박스에 만들어 넣지 않는다. 행의 편집·삭제는 세로 점 메뉴 안에 그대로 유지한다.
...
민트색 타이틀바 아래에 **퀘스트 / 원정 / 캐릭터 / 설정** 탭을 둔다. 선택 탭은 금색과 굵은 글자로 표시한다. 타이틀바와 탭은 고정하고 `.main-content`만 세로 스크롤한다. 핀 버튼은 창의 드래그 이동을 잠그고 항상 위에 표시한다. 고정을 해제하면 타이틀바의 빈 영역·로고·제목에서 드래그할 수 있고 다른 앱을 클릭하면 숨긴다. 초기 고정 상태를 읽거나 변경하는 동안에는 핀 버튼과 드래그를 잠근다. 퀘스트 홈과 원정 보드 모두 `AppHeader`를 사용한다. 톱니 버튼과 설정 탭은 같은 설정 화면으로 연결하고, 테마·앱 동작과 외부 연결을 여기에서 관리한다.

창의 일반 표시 텍스트는 탭 전환이나 드래그 중 선택되지 않는다. 입력·편집 필드의 텍스트 선택은 유지한다.
...
- [ProjectCreateForm](../apps/desktop/src/components/ProjectCreateForm.tsx)의 네이티브 라디오로 기존 폴더와 Git clone을 선택한다. 키보드로 선택할 수 있어야 한다.
- [DirectoryField](../apps/desktop/src/components/DirectoryField.tsx)는 라벨이 있는 경로 입력, 네이티브 폴더 선택, 연결 해제 동작을 함께 제공한다. 선택 취소는 기존 값을 유지한다.
- Git clone은 저장소 주소·저장 위치·새 폴더 이름을 받고, 복제가 끝난 뒤 프로젝트를 연다. 긴 경로는 창을 넓히지 않도록 줄바꿈한다.
- 폴더 선택·복제·저장 중에는 중복 실행을 막고, 진행은 `role="status"`, 실패는 폼 가까운 `role="alert"`로 표시한다. 네이티브 선택창을 닫은 뒤에도 키보드로 폼 작업을 이어갈 수 있어야 한다.
- 기존 입력·버튼·포커스 토큰을 그대로 사용한다. 폴더 선택 기능을 이유로 별도 색상이나 글꼴을 추가하지 않는다.

### 퀘스트·스테이지·진행률

#### Open Design 통합 흐름 (2026-09-15)

구성 원본은 Open Design의 `전체 UI 사용 흐름 검토` 프로젝트(`08f0cb6f-bb1b-437f-a958-0f3114b88351`)에 있는 `queuest-quest-flow.html`이다. 시안의 정적 예시·안내 메시지 동작 대신 실제 저장소와 도메인 전이를 연결한다. 공용 구현은 [QuestFlow.tsx](../apps/desktop/src/components/QuestFlow.tsx), 레이아웃은 [quest-flow.css](../apps/desktop/src/styles/quest-flow.css)에서 관리한다.

- 홈은 `지금의 퀘스트` 제목, 이어갈 퀘스트 카드, 통합 목록 순서다. 날짜 필터가 없으므로 `오늘`이라고 부르지 않는다. 개인 인박스와 원정 작업은 한 목록에 표시하되 저장·완료·보상 규칙은 유지한다.
- 현재 필터 안에서 추적 중인 퀘스트를 먼저 보여주고, 검토 대기·진행 중·대기 순으로 다음 카드를 고른다. 미수락·완료는 대표 카드에서 제외한다. 상태 필터와 원정 선택기는 모든 테마에서 같은 동작을 한다.
- 새 입력은 개인 인박스에 저장한다. 저장 성공 뒤 입력을 비우고 진행할 일·전체 출처 필터로 돌아와 새 항목을 찾을 수 있게 한다. 삭제한 개인 항목의 되돌리기를 유지한다.
- 목록과 대표 카드에서 해당 퀘스트 상세를 바로 연다. 목표·다음 행동·완료 조건·외부 결과물을 표시하고 수락, 시작, 검토 요청, 완료, 되돌리기, 내용 편집을 실제 데이터에 연결한다.
- 완료 검토는 `review` 상태에서만 제공한다. 저장된 성공 조건을 줄 단위로 표시하고 모든 조건 및 `직접 확인했습니다` 항목을 선택해야 완료할 수 있다. 체크는 검토 세션에만 속하며 다시 열면 초기화한다. 조건이 없는 기존 퀘스트도 직접 확인은 필요하다. 저장 실패 시 검토 내용을 유지한다.
- 상세와 앱 설정은 네이티브 HTML `dialog`를 사용해 키보드 포커스를 가두고 Escape·돌아가기·바깥 클릭으로 닫는다. 저장 중에는 닫기와 중복 저장을 막는다. 닫은 뒤 실행한 버튼으로 포커스를 돌려준다.
- 원정 목록은 앱 시작 시 불러온다. 관리 기능은 접힌 워크스페이스 관리에 모으고 원정별 실제 진행률을 표시한다. 원정 보드는 열린 미완료 스테이지를 우선 선택하며 세로 경로 아래에 기존 상태 보드와 편집 기능을 유지한다.
- 375·420px에서는 카드·목록을 세로로, 760px 이상에서는 대표 카드와 목록을 나란히 둔다. 시안의 긴 인라인 상세는 별도 대화상자로 옮겨 좁은 팝오버에서 목록 탐색을 유지한다. 캐릭터는 기존 실제 외형·스킬과 완료 통계 두 카드를 유지한다.
- 전체 목록에서도 원정 그래프의 스테이지 잠금을 따른다. 앞선 빈 스테이지를 포함한 전체 경로로 잠금을 계산하며 잠긴 퀘스트는 상세 열람·편집만 가능하고 다음 상태 진행은 막는다.

브라우저 회귀 검증은 `scripts/ui-flow-smoke.cjs`에서 개발 서버에 격리된 저장소·네이티브 응답 fixture를 주입한다. Playwright가 설치된 환경에서 `node scripts/ui-flow-smoke.cjs`로 실행한다(`UI_SMOKE_URL`, `UI_SMOKE_OUTPUT`, `UI_SMOKE_BROWSER`로 주소·출력·브라우저 채널 변경). 사용자 SQLite·Keychain·외부 계정에 접근하지 않는다. 이 검증은 4가지 기준 크기, 3가지 테마, 완료 검토와 실패 재시도, 수락·편집·시작, 잠긴 스테이지, 개인 퀘스트 추가·삭제·복구, 캐릭터 집계, Escape·포커스 복원을 확인하며 실제 네이티브 앱 검증과는 구분한다.
...
| 기본 테마 | 첨부 레퍼런스의 밝은 모눈·민트 헤더·연결 탭·종이 카드·금색 진행·청록 CTA를 `default-theme.css`에서 적용한다. 기존 레트로와 별빛 모험의 선택값·동작은 유지한다. |
...
| 선택형 판타지 테마 | 420×640 팝오버에서는 6px 좌우 여백과 상단 돌출부가 보이도록 네이티브 창 배경을 투명하게 한다. 기본 레트로 표면은 계속 불투명하다. 판타지 인박스의 CTA는 새 폼을 복제하지 않고 기존 빠른 입력으로 포커스를 이동한다. |
...
`apps/pixel-character-demo`는 기존 캐릭터 탭을 교체하지 않는 별도 기술 데모다. `main`의 `tokens.css`를 재사용한다. 공용 UI 작업과 독립적으로 빌드할 수 있도록 데모에 필요한 `Button`·`Panel`·`Field`·`CheckRow`만 `src/controls.tsx`와 `controls.css`에 두며, 기존 버튼·필드의 시각 계약을 유지한다. 이 데모 컨트롤은 데스크톱 앱의 공용 컴포넌트로 사용하지 않는다. `style.css`는 미리보기와 제어 패널의 배치를 담당하며 별도 팔레트를 정의하지 않는다. 넓은 화면은 미리보기/제어 두 열, 900px 이하는 한 열과 문서 스크롤을 사용한다. 네이티브 팝오버의 420×640 고정 높이는 이 독립 데모에 적용하지 않는다.

실시간 캐릭터 애니메이션은 사용자가 요청한 조작 대상이다. `prefers-reduced-motion`이면 처음에는 정지하고 재생 버튼으로 시작한다. 투명도 체크무늬는 알파 검사용이며 단색으로 전환할 수 있다. 정수 확대 배율을 유지하고 작은 화면에서는 들어가는 최대 배율로 줄인다. 렌더링·동작·체형·측정 제어는 라벨 있는 네이티브 입력과 네 개의 탭으로 나눈다. 3등신 가이드는 명시적으로 켜며, 예시 통계나 완료 카드로 사용하지 않는다.

### 외부 작업 연결 설정

- 설정 탭의 연결 설정에서 Jira·GitHub 프로필과 인증 정보를 관리한다. 저장 직후, 앱 시작 시, 실행 중 5분마다 새 작업을 확인한다. 수동 동기화는 이 화면의 `새로고침` 하나로 제공한다.
- 할 일·원정 보드에는 가져오기·조회 버튼을 두지 않는다. GitHub 리뷰 요청도 일반 퀘스트로 표시한다.
- 동기화 중 상태, 최근 전체 성공 시각, 연결별 실패를 기존 패널·버튼·오류 패턴으로 표시한다. 실패한 연결 때문에 다른 연결의 동기화를 중단하지 않는다.
- 연결별 원정과 퀘스트 스테이지는 새 작업이 있을 때 생성한다. 기존 externalRef는 전체 원정에서 중복 생성하지 않고, 이미 저장된 작업의 편집 내용과 상태는 덮어쓰지 않는다. 외부 완료 작업은 검토 대기로 들어온다.
