...
- 기본값은 이 문서의 레트로 테마다. 타이틀바의 설정 버튼에서 테마를 선택하며, 선택값은 현재 기기의 앱 저장소에 유지한다.
...
민트색 타이틀바 아래에 **할 일 / 원정 / 캐릭터 / 플러그인** 탭을 둔다. 선택 탭은 금색과 굵은 글자로 표시한다. 타이틀바와 탭은 고정하고 `.main-content`만 세로 스크롤한다. 핀 버튼은 창의 드래그 이동을 잠그고 항상 위에 표시한다. 고정을 해제하면 타이틀바의 빈 영역·로고·제목에서 드래그할 수 있고 다른 앱을 클릭하면 숨긴다. 초기 고정 상태를 읽거나 변경하는 동안에는 핀 버튼과 드래그를 잠근다. 인박스와 원정 보드 모두 `AppHeader`를 사용한다.
...
- Git clone은 저장소 주소·저장 위치·새 폴더 이름을 받고, 복제가 끝난 뒤 원정을 연다. 긴 경로는 창을 넓히지 않도록 줄바꿈한다.
...
진행률은 갈색 윤곽 안에 분할선이 있는 막대다. XP는 금색, 원정 진행·스킬은 민트색. 값과 접근성 속성은 실제 도메인 계산을 따른다. 상태 보드는 대기 파랑·진행 중 주황·검토 금색·완료 민트색 헤더와 상태명을 함께 표시한다.
...
| 아이콘 | `clipboard` | `flag` |
| 라벨 | 완료한 퀘스트 | 완료한 스테이지 |
| 값 | 전체 원정의 `done` 퀘스트 수 | 전체 원정의 완료 스테이지 수 |
...
- 개인 인박스 완료는 이 원정 기반 통계나 전역 XP에 임의로 합산하지 않는다.
...
같은 크림색 패널 안에서 짧고 구체적인 설명과 필요한 다음 동작을 보여준다. 빈 화면을 허구의 통계로 채우거나 무관한 장식을 추가하지 않는다. 로딩은 상태 텍스트로 표현하며, 장식용 무한 애니메이션을 만들지 않는다. 플러그인 연결·원정 생성 등 새 기능도 기존 버튼·필드·배지·오류 패턴을 조합한다.
...
| 실제 작업 도구 | 캐릭터 편집, 원정 설정, 저장·취소, 오류 등 실제 기능의 컨트롤은 유지한다. 미리보기와 똑같이 만들기 위해 제거하지 않는다. |
