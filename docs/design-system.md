...
민트색 타이틀바 아래에 **할 일 / 프로젝트 / 캐릭터 / 설정** 탭을 둔다. 선택 탭은 금색과 굵은 글자로 표시한다. 타이틀바와 탭은 고정하고 `.main-content`만 세로 스크롤한다. 핀 버튼은 창의 드래그 이동을 잠그고 항상 위에 표시한다. 고정을 해제하면 타이틀바의 빈 영역·로고·제목에서 드래그할 수 있고 다른 앱을 클릭하면 숨긴다. 초기 고정 상태를 읽거나 변경하는 동안에는 핀 버튼과 드래그를 잠근다. 인박스와 프로젝트 보드 모두 `AppHeader`를 사용한다.
