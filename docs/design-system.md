...
민트색 타이틀바 아래에 **할 일 / 프로젝트 / 캐릭터 / 플러그인** 탭을 둔다. 선택 탭은 금색과 굵은 글자로 표시한다. 타이틀바와 탭은 고정하고 `.main-content`만 세로 스크롤한다. 핀 버튼은 창의 드래그 이동을 잠그고 항상 위에 표시한다. 고정을 해제하면 타이틀바의 빈 영역·로고·제목에서 드래그할 수 있고 다른 앱을 클릭하면 숨긴다. 초기 고정 상태를 읽거나 변경하는 동안에는 핀 버튼과 드래그를 잠근다. 인박스와 프로젝트 보드 모두 `AppHeader`를 사용한다.
...
- Jira 본문은 조회·가져오기에서 제외하고 기존 원본 링크 동작으로 연결한다. 이전에 저장된 Jira 본문도 퀘스트 카드에 펼치지 않는다.
- Jira 알림은 기존 `.editor-panel`, `.section-heading`, `.github-issue-row` 패턴을 사용한다. 최근 변경 목록은 네이티브 `details/summary`로 접고 펼치며 공용 포커스 표시를 적용한다. 원문 열기와 알림 지우기는 기존 버튼을 재사용하며 `.jira-source-link`는 긴 제목 옆에서도 링크 문구가 한 줄로 유지되게 한다.
- 마지막 조회 시간·알림 수는 `role="status"`, 연결별 조회 실패는 `role="alert"`로 표시한다. OS 알림이 꺼져 있어도 앱 안에서 최근 변경을 확인할 수 있다.
