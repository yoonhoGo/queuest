...
macOS 27 이상에서는 상태 항목에 native 메뉴를 연결할 때 클릭 이벤트가 잘못 전달되는
[tray-icon 이슈 #355](https://github.com/tauri-apps/tray-icon/issues/355)를 우회합니다. 이
환경에서는 좌·우 클릭 모두 Queuest 팝오버를 열고, macOS 26 이하에서는 기존 우클릭 native
메뉴를 유지합니다.

- 상단의 **할 일 / 프로젝트 / 캐릭터 / 설정** 메뉴는 스크롤해도 유지됩니다.
...
- 설정 화면에서 연결과 로컬 도구를 관리합니다. Jira 티켓, GitHub 이슈와 PR 리뷰 요청은 저장된 연결을 통해 주기적으로 확인하고 일반 퀘스트로 표시합니다. 수동 새로고침은 연결 설정에서 제공합니다. Calendar·EventKit 항목 가져오기는 후속 범위입니다.
...
## Jira·GitHub 백그라운드 작업 확인

- **설정 → 연결 설정**에 Jira 또는 GitHub 연결을 저장합니다. Jira는 사이트·이메일·API token·프로젝트 키, GitHub는 저장소(`owner/repository`)·PAT를 사용합니다. 인증 정보는 macOS Keychain에 보관합니다.
- 앱 시작, 연결 저장 직후, 앱 실행 중 5분마다 새 작업을 읽습니다. 메뉴바 팝오버를 닫아도 앱이 실행 중이면 계속 확인하며 앱 종료·기기 잠자기 중 실행은 보장하지 않습니다. 설정에서 **새로고침**, 최근 전체 성공 시각과 연결별 오류를 확인합니다.
- Jira는 설정된 프로젝트 티켓을 페이지별로 조회합니다. 보드 ID가 있으면 백로그를 미수락 퀘스트로 보존합니다. GitHub는 저장소 이슈와 해당 저장소에서 나에게 요청된 열린 PR 리뷰를 조회합니다.
- 새 작업은 연결별 원정의 `퀘스트` 스테이지에 저장합니다. 전체 원정에서 `externalRef`로 중복을 막으며, 이미 저장된 작업은 사용자가 편집한 내용·위치·완료 상태를 유지합니다. 따라서 외부 변경을 기존 퀘스트에 덮어쓰는 양방향 동기화는 아닙니다. 삭제한 외부 퀘스트는 다음 조회에서 다시 나타날 수 있습니다.
- 외부에서 완료된 Jira·GitHub 이슈도 **검토 대기**로 저장하고 완료는 사용자가 확인합니다. PR 리뷰 요청은 대기 퀘스트로 저장됩니다.
- 한 번에 하나의 동기화만 실행합니다. 연결별 최대 100페이지, 페이지당 최대 100개를 조회하며 실패 전 저장된 항목은 재시도해도 중복되지 않습니다.

네이티브 Host는 고정 HTTPS API, 리다이렉트 금지, 요청당 30초 제한과 8 MiB 응답 제한을 사용합니다. API token이나 원격 오류 본문은 화면에 반환하지 않습니다. GitHub 연결 동기화는 작업 폴더나 `gh` 인증을 사용하지 않습니다.
