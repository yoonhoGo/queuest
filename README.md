...
현재 레포는 `Tauri 2 + React + TypeScript + SQLite` 기반의 MVP 골격입니다. 앱은 SQLite 인박스를 먼저 열고, 사용자가 프로젝트를 선택하거나 만들 때만 워크스페이스·프로젝트·마일스톤·태스크 그래프를 불러옵니다. 선택한 프로젝트 보드의 상태 이동은 로컬 SQLite에 즉시 저장되며, AI·GitHub 흐름은 아직 외부 경계만 준비되어 있습니다.
