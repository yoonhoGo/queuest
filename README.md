...
개인의 일을 워크스페이스 → 원정 → 스테이지 → 퀘스트로 관리하는 macOS 메뉴바 앱입니다.

현재 레포는 `Tauri 2 + React + TypeScript + SQLite` 기반의 로컬 MVP입니다. 앱은 SQLite 인박스와 원정 퀘스트 목록을 먼저 열고, 원정 보드나 전역 캐릭터 탭에서 필요할 때만 전체 워크스페이스·원정·스테이지·퀘스트 그래프를 불러옵니다. 워크스페이스와 원정·스테이지·퀘스트 CRUD, 보드 상태 이동은 로컬 SQLite에 즉시 저장되고, 퀘스트 댓글·전역 캐릭터·원정 장비 설정도 재실행 시 복원할 수 있는 저장 경계를 갖습니다. 메뉴바 팝오버는 포커스와 핀 상태를 반영하며 도구 상태를 확인합니다. 외부 연동은 `@queuest/plugin-contracts`의 버전 있는 프로토콜과 [플러그인 아키텍처](./docs/plugin-architecture.md)를 기준으로 확장합니다.

## 미리보기

macOS 메뉴바에서 Queuest 아이콘을 클릭하면 아이콘 바로 아래에 팝오버가 열립니다. 우클릭 메뉴로 열어도 같은 위치를 사용하며, 화면 좌우 경계를 넘지 않도록 배치합니다.

- 상단의 **할 일 / 원정 / 캐릭터 / 플러그인** 메뉴는 스크롤해도 유지됩니다.
- 캐릭터 이름과 직업은 **캐릭터** 탭에서 전역으로 편집합니다. 경험치·레벨·스킬은 모든 원정의 완료 기록을 합산하고, 원정별 장비 설정은 원정 설정에 남습니다.
- **할 일** 탭에서 개인 인박스와 모든 원정의 퀘스트를 함께 확인할 수 있습니다. 원정 퀘스트에서 원정 보드와 외부 원본 링크로 바로 이동합니다.
- 플러그인 화면에서 로컬 도구의 설치·인증 상태를 다시 확인할 수 있습니다. GitHub는 원정의 작업 폴더와 `gh` 인증으로 이슈를 가져오며, 할 일 화면에서 나에게 리뷰 요청된 PR을 조회할 수 있습니다. Jira는 저장된 연결로 내 담당 티켓과 보드 백로그를 가져옵니다. Calendar·EventKit 항목 가져오기는 후속 범위입니다.
...
- GitHub connector는 `com.queuest.github` plugin namespace와 `connectionId`를 조합한
  Credential Store 항목을 읽고, GitHub REST GET만 수행합니다. 앱에서는 원정의
...
- **연동 → Jira 연결**에 사이트 URL·이메일·API token을 저장합니다.
...
- **원정 → 스테이지 → Jira 가져오기**에서 연결·범위·저장할 스테이지를 선택하고
  **접근 허용하고 조회**를 누릅니다. 프로젝트 제한 없이 `assignee = currentUser()`인
  티켓만 최근 업데이트 순으로 표시합니다. 새 티켓을 선택한 뒤 가져옵니다. 한 페이지는 최대
  100개이며 **다음 페이지**로 이어서 조회합니다. 조회된 티켓만 선택·저장됩니다.
- Jira와 GitHub 연결을 저장하면 첫 동기화를 즉시 실행하고, 메뉴바 앱이 실행되는 동안
  5분마다 다시 동기화합니다. Jira Epic은 `Jira · <연결 이름>` 워크스페이스의 원정으로,
  Epic 하위 티켓은 해당 원정의 퀘스트로 가져옵니다. Epic이 조회 결과에 없어도 parent key로
  원정을 만들며, parent가 없는 티켓은 연결별 `내 Jira 퀘스트` 원정에 모읍니다.
- Jira의 해결된 티켓과 GitHub의 닫힌 이슈는 완료 퀘스트로 가져옵니다. 이후 동기화에서는
  같은 `externalRef`를 새로 만들지 않고 제목·본문·상태를 갱신합니다.
- **보드 백로그 → 미수락 퀘스트** 범위는 보드의 실제 백로그 중 내 담당 티켓만
  조회합니다. 보드 ID가 설정되어 있으면 내 담당 티켓 범위에서도 해당 보드의 백로그를
...
- 일반 Jira 티켓은 시작 전 → 대기, 진행 중 → 진행 중, 완료 → 검토 대기로 매핑합니다.
  완료는 기존 사람의 확인 절차를 따릅니다. 동일 원정 내 전체 스테이지에서
  `jira:<tenant>.atlassian.net/<issueKey>`로 중복을 막습니다. 다른 원정 간 병합이나
