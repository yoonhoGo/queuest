...
2026-09-11 승인된 [레트로 웹 미리보기](previews/retro/dist/index.html)의 시각 언어는 계속 유지한다. 새 설치의 [기본 테마](apps/desktop/src/styles/default-theme.css)는 첨부 레퍼런스의 파란 모눈 배경, 밝은 종이 카드, 민트색 타이틀바, 연결된 탭, 금색 선택 상태와 짙은 청록색 주요 동작을 적용한다. 기존 `retro`와 `fantasy` 선택값 및 동작은 보존한다. 원본의 카드 구조와 글자 위계도 디자인 기준에 포함된다.
