  ...
  useEffect(() => {
    ...
    return () => {
      ...
      queueMicrotask(() => {
        ...
        else document.querySelector<HTMLElement>('.quest-home-heading h2, #project-title')?.focus();
      });
    };
  }, []);
  return <dialog ref={ref} className="quest-dialog" aria-label={title}
    ...
    <div className="quest-dialog-content">
      <button type="button" className="back-link" disabled={busy} onClick={onClose}>← 돌아가기</button>
      {children}
    </div>
  </dialog>;
}
...
export function QuestDetail({ item, onSave, onClose, onOpenSource, onOpenProject, stageLocked = false }: {
  ...
  return <QuestDialog title={reviewing ? "퀘스트 완료 검토" : "퀘스트 상세"} busy={busy} onClose={onClose}>
    ...
    <span className={`quest-state ${pending ? "pending" : task.status}`}>{QUEST_STATUS[pending ? "pending" : task.status]}</span>
    <h2>{task.title}</h2>
    {draft ? <form onSubmit={event => { event.preventDefault(); if (draft.title.trim()) void save({ ...draft, title: draft.title.trim() }); }}>
      <fieldset disabled={busy} className="quest-edit-fields">
        ...
        <div className="form-actions"><button className="primary-button" type="submit">저장</button><button className="secondary-button" type="button" onClick={() => setDraft(null)}>편집 취소</button></div>
      </fieldset>
    </form> : <>
      ...
      <section className="quest-detail-section"><h3>완료 조건</h3>
        {criteria.length ? criteria.map(criterion => reviewing ? <label className="quest-criterion" key={criterion}>
          <input type="checkbox" disabled={busy} checked={checked.includes(criterion)} onChange={event => setChecked(current => event.target.checked ? [...current, criterion] : current.filter(value => value !== criterion))} />
          <span>{criterion}</span></label> : <p key={criterion}>{criterion}</p>) : <p>등록된 완료 조건이 없습니다. 작업 내용과 결과를 직접 확인하세요.</p>}
      </section>
      ...
      {reviewing && <div className="quest-review-note">
        ...
        <label className="quest-criterion"><input type="checkbox" checked={confirmed} disabled={busy} onChange={event => setConfirmed(event.target.checked)} /><span>작업 결과와 완료 조건을 직접 확인했습니다.</span></label>
      </div>}
      ...
      <div className="quest-detail-actions">
        {pending ? <button className="primary-button" disabled={busy} onClick={() => void save({ ...task, status: "todo", quest: { ...task.quest!, acceptance: "accepted" } })}>퀘스트 수락</button>
          : task.status !== "done" && <button className="primary-button" disabled={busy || stageLocked || (reviewing && !canComplete)} onClick={() => move(advanceTaskStatus(task.status))}>
            {busy ? "저장 중…" : reviewing ? "완료 확인" : task.status === "todo" ? "퀘스트 시작" : "검토 요청"}</button>}
        {!pending && task.status !== "todo" && <button className="secondary-button" disabled={busy} onClick={() => move(retreatTaskStatus(task.status))}>{reviewing ? "작업으로 돌아가기" : task.status === "done" ? "검토로 되돌리기" : "대기로 되돌리기"}</button>}
      </div>
      <div className="quest-secondary-actions"><button type="button" className="row-action" disabled={busy} onClick={() => setDraft({ ...task, quest: task.quest ?? createQuestContext() })}>내용 편집</button>
        {onOpenProject && <button type="button" className="row-action" disabled={busy} onClick={() => onOpenProject(item.project.id)}>원정 보드 열기</button>}</div>
    </>}
...
export function QuestHome({ headingRef, todos, projectTodos, projectGraphs, projectTodoLoadError, actionError, deletedTodo,
  ...
  return <div className="quest-home">
    ...
    {featured && <section className="quest-featured" aria-label="지금 이어갈 퀘스트">
      <div className="quest-row-meta"><span className={`quest-state ${featured.status}`}>{QUEST_STATUS[featured.status]}</span><span>{featured.item?.task.quest?.tracked ? "추적 중" : "이어서 할 일"}</span></div>
      ...
      <button type="button" className="primary-button" onClick={() => open(featured)}>{featured.status === "review" ? "검토 열기" : featured.status === "doing" ? "퀘스트 이어하기" : "퀘스트 열기"}</button>
    </section>}
    <section className="quest-queue" aria-labelledby="quest-queue-title">
      ...
      <div className="quest-filters" role="group" aria-label="퀘스트 상태 필터">
        {([['active', '진행할 일'], ['all', '전체'], ...Object.entries(QUEST_STATUS)]).map(([value, label]) => <button className="small-button" type="button" key={value} aria-pressed={filter === value} onClick={() => setFilter(value)}>{label}</button>)}
      </div>
      ...
      <form className="quest-capture" onSubmit={event => { event.preventDefault(); if (!title.trim()) { setError("퀘스트 제목을 입력하세요."); return; }
        ...
        <button type="submit" className="primary-button" disabled={busy}>추가</button>
      </form>
      ...
      <div className="quest-queue-list">{visible.map(entry => <button type="button" className={`quest-queue-row ${entry.status}`} key={entry.key} onClick={() => open(entry)}>
        <span className="quest-row-meta"><span className={`quest-state ${entry.status}`}>{QUEST_STATUS[entry.status]}</span><span>{entry.item?.task.assignee === "ai" ? "AI" : "나"}</span></span>
    ...
    {deletedTodo && <div className="undo-bar" role="status"><span>“{deletedTodo.title}” 삭제됨</span><button type="button" disabled={busy} onClick={() => void mutate(onRecover, () => undefined)}>되돌리기</button></div>}
    ...
    {selectedEntry?.todo && <QuestDialog title="개인 퀘스트 상세" busy={busy} onClose={() => setSelected(null)}>
      ...
      <form onSubmit={event => { event.preventDefault(); if (!editTitle.trim()) { setError("퀘스트 제목을 입력하세요."); return; } void mutate(() => onUpdate({ ...selectedEntry.todo!, title: editTitle.trim() }), () => setSelected(null)); }}>
        ...
        <div className="form-actions"><button type="submit" className="secondary-button" disabled={busy}>제목 저장</button><button type="button" className="primary-button" disabled={busy} onClick={() => void mutate(() => onUpdate({ ...selectedEntry.todo!, completed: !selectedEntry.todo!.completed }), () => setSelected(null))}>{selectedEntry.todo.completed ? "대기로 되돌리기" : "완료 확인"}</button></div>
      </form>
      <button type="button" className="row-action danger" disabled={busy} onClick={() => void mutate(() => onDelete(selectedEntry.todo!), () => setSelected(null))}>퀘스트 삭제</button>
      {error && <p role="alert" className="action-error">{error}</p>}
    </QuestDialog>}
  </div>;
}
