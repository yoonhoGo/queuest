...
test("runs the Jira plugin through the manager process boundary after approval", async () => {
  const pluginDirectory = fileURLToPath(new URL("../../plugin-jira", import.meta.url));
  const pluginId = "com.queuest.jira";
  const requestedPermissions = {
  ...
  assert.equal(spawnCalls.length, 1);
  assert.equal(spawnCalls[0].command, "node");
  assert.deepEqual(spawnCalls[0].args, ["--experimental-strip-types", "bin/queuest-jira.mjs"]);
  assert.notEqual(spawnCalls[0].options.shell, true, "PluginManager must launch without a shell");
  ...
  // Jira credentials or contact Jira while proving it is initialized.
  assert.deepEqual(
    await manager.requestPlugin<{ state: string }>(pluginId, "health.check", {}),
    { state: "connected" },
  );
